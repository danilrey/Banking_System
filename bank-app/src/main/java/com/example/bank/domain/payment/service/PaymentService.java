...
public class PaymentService {
    ...
    public Payment scheduleFromAccount(Long accountId, BigDecimal amount, String currency, PaymentCategory category, String providerName, String detailsJson, OffsetDateTime when) {
        ...
        Payment payment = Payment.builder()
                ...
                .amount(amount)
                .currency(currency)
                .status(PaymentStatus.SCHEDULED.name())
                .scheduledAt(when)
                .createdAt(OffsetDateTime.now())
                .build();
    ...
    public Payment executeScheduledPayment(Long paymentId) {
        ...
        if (!PaymentStatus.SCHEDULED.name().equals(payment.getStatus())) {
            throw new IllegalStateException("Payment is not scheduled: " + paymentId);
        }

        PaymentCategory category = PaymentCategory.valueOf(payment.getCategory());
    ...
    public List<Payment> getCustomerPaymentsByStatus(CustomerProfile customer, PaymentStatus status) {
        return paymentRepository.findByCustomerAndStatus(customer, status.name());
    }

    @Transactional(readOnly = true)
    public List<Payment> getPaymentsByCategory(PaymentCategory category) {
        return paymentRepository.findByCategory(category.name());
    }
    ...
    private PaymentType resolveType(PaymentCategory category) {
        return paymentTypes.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Unsupported payment category: " + category));
    }
}
