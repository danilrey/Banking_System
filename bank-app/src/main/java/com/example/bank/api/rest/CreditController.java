...
public class CreditController {
    ...
    public CreditController(CreditService creditService, CustomerProfileRepository customerProfileRepository) {
    ...
    public ResponseEntity<CreditResponse> createCredit(
        ...
        Credit credit = creditService.createCredit(customer, request.getPrincipalAmount(), request.getCurrency(), request.getInterestRateAnnual(), request.getTermMonths(), request.getCreditType());
        CreditResponse response = mapToResponse(credit);
    ...
    @PreAuthorize("hasRole('USER')")
    @ResponseBody
    public ResponseEntity<List<CreditResponse>> getCredits(@AuthenticationPrincipal CustomerProfile customer) {
        List<Credit> credits = creditService.getCreditsByCustomer(customer);
        List<CreditResponse> responses = credits.stream().map(this::mapToResponse).toList();
    ...
    public ResponseEntity<CreditResponse> getCredit(@PathVariable Long id) {
        ...
        CreditResponse response = mapToResponse(creditOpt.get());
    ...
    public ResponseEntity<List<CreditResponse>> getCreditsByStatus(@PathVariable CreditStatus status,
                                                                   @AuthenticationPrincipal CustomerProfile customer) {
        List<Credit> credits = creditService.getCreditsByCustomer(customer).stream()
    ...
    public String getCreditsPage(Model model, @AuthenticationPrincipal CustomerProfile customer) {
        List<Credit> credits = creditService.getCreditsByCustomer(customer);
    ...
    private CustomerProfile requireCustomer(String username) {
        if (username == null) {
            throw new AccessDeniedException("Authentication required");
        }
        return customerProfileRepository.findByPhone(username)
                .orElseThrow(() -> new AccessDeniedException("Customer profile not found for username: " + username));
    }
}
